...
public:
    ...
    void join_pass_perk(std::shared_ptr<dexode::eventbus::Bus> bus) {
        ...
        addPerk(std::make_unique<dexode::eventbus::perk::PassEverythingPerk>(bus)).registerPrePostpone(&dexode::eventbus::perk::PassEverythingPerk::onPrePostponeEvent);
    }

    template<template<typename> typename T>
    std::shared_ptr<dexode::eventbus::perk::TagPerk<T>> join_tag_perk(std::shared_ptr<dexode::eventbus::Bus> bus,std::string key) {
...
inline PerkEventBus::RegisterHelper PerkEventBus::addPerk(std::shared_ptr<Perk> perk) {
    auto* local = perk.get();
    _perks.push_back(std::move(perk));
    return RegisterHelper(this, local);
}

inline bool PerkEventBus::postponeEvent(PostponeHelper& postponeCall) {
