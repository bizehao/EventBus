...
Team::Team(std::shared_ptr<EventBus> bus) : _bus(std::move(bus)) {}
...
//        tagPerk->wrapTag<event::TagEvent<event::Test_Data>>();
//        characterBus->addPerk(std::move(tagPerk)).registerPrePostpone(&dexode::eventbus::perk::TagPerk::onPrePostponeEvent);
    }
